...
file = fountain.read_sandbox_file(sandbox_id, "/work/repository/build.log")
print(file.data.decode())
```

Both calls require a ready sandbox and a full-scope key. Reading or running a
command is allowed while an agent is working, so a command that writes also
changes the files beneath that agent.
