...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/sandboxes/s-1/files":
            raw = b"hello\x00world"
            self.send_response(200)
            self.send_header("Content-Type", "application/octet-stream")
            self.send_header("Content-Length", str(len(raw)))
            self.send_header("X-Fountain-Truncated", "true")
            self.end_headers()
            return self.wfile.write(raw)
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/sandboxes/s-1/exec":
            return self._json(
                200,
                {
                    "data": {
...
class ClientTests(unittest.TestCase):
    ...
    def test_sandbox_exec_and_binary_file(self):
        with FakeFountain() as fake:
            client = Fountain(base_url=fake.base_url, api_key="fk_test")
            result = client.exec_sandbox(
                "s-1", "git", args=["status"], cwd="/work", timeout_ms=1_000
            )
            self.assertEqual(result["exit_code"], 0)
            command = next(
                request
                for request in fake.state.requests
                if request[:2] == ("POST", "/api/sandboxes/s-1/exec")
            )
            self.assertEqual(
                command[3],
                {
            ...
            file = client.read_sandbox_file("s-1", "/work/a.bin")
            self.assertEqual(file.data, b"hello\x00world")
