defmodule Fountain.Conversations.ConversationServer do
      ...
      case TurnMachine.gate(conv, state.inference_source) do
        :ok ->
          ...
          case kick_turn(state, prompt, agent, images) do
            ...
            {:error, reason} ->
              Logger.info(
                "conv #{state.conversation_id}: initial turn refused (#{inspect(reason)})"
              )
  ...
  defp kick_turn(state, prompt, agent, images) do
    ...
    case TurnMachine.open(state.conversation_id, state.sandbox_id, prompt) do
      {:ok, conv, turn} -> {:ok, run_turn(state, conv, turn, prompt, agent, images)}
      :at_capacity -> {:error, :sandbox_at_capacity}
      {:error, _} = error -> error
    end
  end
  ...
  defp run_fresh_turn(state, conv, turn, prompt, agent, images, acp?) do
    Fountain.Conversations.TurnLaunch.run(
      state,
      conv,
      turn,
      prompt,
