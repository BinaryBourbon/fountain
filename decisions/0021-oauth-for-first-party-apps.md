...
verified: { by: human:jhgaylor, at: 2026-08-29T00:00:00-04:00 }
...
- **Development mode is the security boundary, not the redirect allowlist.**
  ...
  *before* the redirect check so a stranger's error page discloses no
  ...
  the person who registered it, its owner may name any redirect URI at all.
  `published` is an operator flip with no self-serve path.
