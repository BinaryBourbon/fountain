defmodule Fountain.Workers.CreditExpirerTest do
  @moduledoc """
  ...
  test "an unspent grant expires in full, once; a spent one expires nothing" do
    ...
    {:ok, _} =
      Credits.grant(full.id, 1000, "grant_opening", idempotency_key: "f", expires_at: @expires)

    {:ok, _} =
      Credits.grant(spent.id, 1000, "grant_opening", idempotency_key: "s", expires_at: @expires)

    {:ok, _} = Credits.debit(spent.id, 1000, "burn_turn", idempotency_key: "spent-all")

    assert %{expired: 0} = CreditExpirer.run(now: @day_before)
    assert %{expired: 1} = CreditExpirer.run(now: @day_after)
    ...
    assert %{expired: 0} = CreditExpirer.run(now: DateTime.add(@day_after, 86_400, :second))
  end
  ...
  test "an expiry never reaches purchased money, even when the grant was spent after the sweep looked" do
    ...
    assert Credits.unspent_of(grant) == 0
  end
