...
| `BROKER_LOG_RETENTION_HOURS` | `168` | — | How long a conversation's vault on the broker keeps its request log after the conversation ends. `GET /api/conversations/:id/egress` reads it. A daily job deletes older vaults. Keep it at or below the broker's own log retention. The native backend applies the same retention to the `broker_requests` rows it writes. |
