...
| `TURN_EXECUTION_LIMIT_CEILING` | `{}` | — | JSON object of per-turn host ceilings: `wall_time_seconds`, `max_model_turns`, and `max_estimated_cost_usd`. Invalid values refuse boot. Leave unset during the [execution-limit preview](api.md#execution-limits-preview): nonempty effective limits refuse work until enforcement is integrated. Estimated cost is not a billed-cost cap. |
