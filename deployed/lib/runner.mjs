...
export const profiles = { probe, basic, execution, streaming: execution, secrets, mcp, deterministic };
...
export function configFrom(path, env = process.env) {
  ...
  const allowed = ['base_url', 'credentials', 'profiles', 'contract', 'required_capabilities', 'optional_capabilities', 'limits', 'execution', 'deployment', 'secrets', 'mcp', 'fixture'];
  ...
  if (config.profiles.some(name => ['basic', 'execution', 'streaming', 'secrets', 'mcp', 'deterministic'].includes(name))) {
