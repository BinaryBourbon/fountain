...
| NC-9 | ~~**`usage_events` has no `on_delete` FK**~~ **RESOLVED 2026-08-01** | Migration `20260801160000` | Was: deleting a user failed at DB level. Now `on_delete: :nilify_all` with a nullable column, so usage history survives without naming anyone. Account deletion is implemented in `Fountain.Accounts.Deletion` (#170). |
