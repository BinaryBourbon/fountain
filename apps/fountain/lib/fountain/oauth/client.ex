defmodule Fountain.OAuth.Client do
  ...
  which is what makes the next sentence safe: the owner may register any
  redirect URI they like — their sprite's public URL, a `localhost` port,
  anything — because no one else's account is reachable through it.
  ...
  `published` is not castable either — see the moduledoc.
  """
  def changeset(client, attrs) do
    client
    |> cast(attrs, [:name, :redirect_uris, :user_id])
    |> put_client_id()
  ...
  def origin_key(origin) when is_binary(origin) do
    case URI.parse(origin) do
      %URI{scheme: scheme, host: host} when is_binary(scheme) and is_binary(host) ->
        if loopback?(host), do: "#{scheme}://#{String.downcase(host)}", else: origin_of(origin)
