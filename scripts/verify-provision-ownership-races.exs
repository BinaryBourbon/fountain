...
alias Fountain.Conversations.{ExecutionGuard, LogEvent, ProvisionWatchdog, Sandbox, Turn, TurnExecution}
import Ecto.Query

for operation <- [:startup, :watchdog] do
  ...
  waiter = Task.async(fn ->
    Repo.checkout(fn ->
      ...
      case operation do
        :startup -> ExecutionGuard._unsafe_interrupt_on_sandbox(parent.id, source.id)
        :watchdog -> ProvisionWatchdog._unsafe_expire(parent.id, source.id)
      end
    end)
  end)
  ...
  case operation do
    :startup -> {:error, :ownership_changed} = Task.await(waiter, 10_000)
    :watchdog -> {:ok, :stale} = Task.await(waiter, 10_000)
  end
...
IO.puts("PROVISION_OWNERSHIP_RACE_RESULT=" <> Jason.encode!(%{
  separate_database_connections: true,
  forced_lock_waits: 2,
  operations: ["startup_interrupt", "provision_watchdog"],
  replacement_execution_preserved: true,
  stale_failure_events: 0,
  provider_operations: 0,
  scope: "Local transfer commits and replacement admission before a stale worker acquires its original machine lock; does not prove actor epochs or prompt delivery"
}))
