defmodule Fountain.Conversations.ExecutionLimits do
  ...
  invalid. Only the operator's dedicated account-setting path may clear a ceiling.

  `execution_limits` on a conversation is its saved launch allowance, which may narrow. A new
  turn intersects that allowance with current host/account ceilings; a policy
  ...
  @doc "The operator's configured ceiling; validation happens at every admission."
  def host_ceiling, do: Application.get_env(:fountain, :execution_limit_ceiling, %{})

  @doc """
  Controls enforced by the integrated runtime transport.

  Admission stays closed for nonempty limits until the identity transport and
  deadline worker are wired. Publishing an SDK option or a journal table alone
  must never make the public API promise enforcement. This empty capability set
  is a rollout fence, not a user-configurable bypass.
  """
