...
| `BROKER_URL` | — | — | The management API of an [Agent Vault](https://github.com/Infisical/agent-vault) egress broker, for example `http://agent-vault:14321`. This selects the Agent Vault backend of credential brokerage (ADR 0019). Set it, and boot also requires `BROKER_TOKEN` and `BROKER_PROXY_URL`. Blank turns this backend off. When `BROKER_LISTEN_PORT` is also blank, credential brokerage is off and each conversation provisions as before. Fountain brokers the tenants in `BROKER_TENANTS` only. Boot refuses `BROKER_URL` together with `BROKER_LISTEN_PORT`. |
| `BROKER_LISTEN_PORT` | — | — | A port number, for example `14322`. This selects the native backend: Fountain runs the egress proxy itself, from the `managoat_broker` library, on this port on every replica. Each conversation's proxy session goes in the `broker_sessions` table, so any replica can serve a sandbox. Set it, and boot also requires `BROKER_PROXY_URL`. Point your ingress at this port. The proxy speaks plain HTTP, and TLS towards the sandbox is the job of the ingress. Boot refuses `BROKER_LISTEN_PORT` together with `BROKER_URL`. |
| `BROKER_TOKEN` | — | With `BROKER_URL`. | A session token of an Agent Vault member. Fountain uses it to create a vault, to load credentials and to mint a proxy session for each conversation. The native backend does not read it. |
| `BROKER_PROXY_URL` | — | With `BROKER_URL` or `BROKER_LISTEN_PORT`. | The proxy address a sandbox dials, for example `http://broker.example.com:14322`. It is not the same as `BROKER_URL` when the sandboxes are on a different network from Fountain. Its host is the one host a brokered sandbox may reach. |
...
| `BROKER_LOG_RETENTION_HOURS` | `168` | — | Agent Vault only. How long a conversation's vault on the broker keeps its request log after the conversation ends. `GET /api/conversations/:id/egress` reads it. A daily job deletes older vaults. Keep it at or below the broker's own log retention. The native backend keeps no stored request log yet. It writes one log line per request, and the endpoint returns an empty page. |
...
| `SANDBOX_QUEUE_MAX_DEPTH` | `10` | No. | The most sandbox requests one tenant may have waiting or starting. Requests beyond it keep the immediate capacity error. |
| `SANDBOX_QUEUE_MAX_WAIT_SECONDS` | `3600` | No. | How long a sandbox request may wait for capacity before it expires. |
