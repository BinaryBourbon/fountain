defmodule Fountain.Conversations.ConversationReapplyTest do
  ...
  describe "applying a selection to the machine that is already there" do
    ...
    test "tells the live server to apply it, and keeps the runtime session", ctx do
      ...
      stub(Fountain.Conversations.ConversationServer, :refresh_configuration, fn id, _revision ->
  ...
  describe "binding and turn admission regressions" do
    test "legacy skill cleanup keeps the pre-reapply version as its source", ctx do
    ...
    test "a stale server cannot open a turn after reapply", ctx do
      assert {:ok, updated} = Conversations.reapply_conversation(ctx.conv, %{})
    ...
    test "refresh failures are returned without a successful lifecycle event", ctx do
      Mimic.expect(Fountain.Conversations.ConversationServer, :refresh_configuration, fn _, _ ->
        {:error, :provisioning}
      end)

      assert {:error, :provisioning} = Conversations.reapply_conversation(ctx.conv, %{})
