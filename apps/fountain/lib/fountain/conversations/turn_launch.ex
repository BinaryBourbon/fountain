defmodule Fountain.Conversations.TurnLaunch do
  @moduledoc """
  Launch a fresh command for an already admitted conversation turn.

  Bounded turns use the registered journal and supervised transport; unbounded
  turns retain their existing spawn path. The actor supplies its output writer
  for pre-start failure diagnostics and keeps ownership of the returned state.
  """
  require Logger
  require OpenTelemetry.Tracer

  alias Fountain.Conversations
  alias Fountain.Conversations.{Connection, McpServers, Output, TurnMachine}
  ...
  def run(state, conv, turn, prompt, agent, images, acp?, output_writer) do
    case TurnMachine.session_plan(turn, state.runtime_session_id) do
      {:ok, plan} ->
        launch(state, conv, turn, prompt, agent, images, output_writer, %{
          acp?: acp?,
          session_plan: plan
        })
    ...
    try do
      ...
      spawn_result =
        if state.turn_execution do
          # ownership: admission registered this actor's turn against its persisted tenant and sandbox.
      ...
      case spawn_result do
        ...
        {:error, reason} ->
          fail_turn_before_start(state, turn, reason, "spawn failed", output_writer)
      end
    after
  ...
  #
  # Called only from inside run's try block, which restores the caller's
  # previous current-span in its `after`; the span this ends is the turn span
  # run opened a few lines above the call.
  defp fail_turn_before_start(
         state,
         turn,
         reason,
         what,
  ...
  defp bounded_sdk_limits(state, runtime) do
    options =
      Fountain.Conversations.ExecutionLimits.sdk_options(state.turn_execution.execution_limits)
